...
export default function ProgressCard({ percentual = 0 }: ProgressCardProps) {
  return (
    <View className="w-96 h-64 bg-white rounded-3xl">
      <Text className="pt-8 pl-10 font-semibold text-lg">
        Sessões realizadas
      </Text>
      <Text className="pt-5 pl-10 font-semibold text-lg color-[#0048FF]">
        {(percentual * 100).toFixed(0)}%
      </Text>
      {/* barra de status */}
      <View className="w-80 h-6 bg-[#E5EAF2] rounded-lg mt-5 ml-10 justify-center">
        <View
          className="h-5 bg-[#3A7DFF] rounded-lg"
